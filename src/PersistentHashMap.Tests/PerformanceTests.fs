module PersistentHashMap.Tests.PerformanceTests
...
open PersistentHashMap.Tests.Common
...
#if DEBUG
let total, inner = 40000, 100
#else
let total, inner = if zoom_in then 100000000, 1000 else 4000000, 100
#endif
...
let inserts =
  [| for i in 0 .. (inner - 1) ->
       random 0 (inner * multiplier) |> makeKey, string i |]
...
module TestDictionary =
  ...
  let inline dictContainsKey k (d : Dictionary<_, _>) = d.ContainsKey k

  let inline doInsert hm =
    inserts |> Array.fold (fun s (k, v) -> s |> dictAdd k v) hm

  let inline doRemove hm =
    removals |> Array.fold (fun s (k, _) -> s |> dictRemove k) hm

  let inline doLookup fa hm =
    fa |> Array.forall (fun (k, _) -> hm |> dictContainsKey k)
  ...
  let insert () =
    ...
    Checker.check
      (fun () -> result.Count = inserted.Count)
      "Expected to be same length as testSet"
...
module TestHashtable =
  ...
  let inline htContainsKey k (d : Hashtable) = d.ContainsKey k

  let inline doInsert hm =
    inserts |> Array.fold (fun s (k, v) -> s |> htAdd k v) hm

  let inline doRemove hm =
    removals |> Array.fold (fun s (k, _) -> s |> htRemove k) hm

  let inline doLookup fa hm =
    fa |> Array.forall (fun (k, _) -> hm |> htContainsKey k)
...
module TestPersistentHashMap =
  open PersistentHashMap

  let inline doInsert phm =
    inserts |> Array.fold (fun s (k, v) -> PersistentHashMap.set k v s) phm

  let doRemove phm =
    removals |> Array.fold (fun s (k, v) -> PersistentHashMap.unset k s) phm

  let inline doLookup fa phm =
    fa |> Array.forall (fun (k, _) -> PersistentHashMap.containsKey k phm)
  ...
  let insert () =
    ...
    Checker.check
      (fun () ->
        PersistentHashMap.length result = PersistentHashMap.length inserted)
      "Expected to be same length as testSet"
  ...
  let remove () =
    ...
    Checker.check
      (fun () -> PersistentHashMap.isEmpty result)
...
module TestSCI =
  ...
  let inline doInsert hm =
    inserts
    |> Array.fold
      (fun (s : ImmutableDictionary<_, _>) (k, v) -> (s.Remove k).Add (k, v))
      hm
  ...
  let inline doLookup fa (hm : ImmutableDictionary<_, _>) =
    fa |> Array.forall (fun (k, _) -> hm.ContainsKey k)
...
module TestMap =
  ...
  let inline doInsert hm =
    inserts |> Array.fold (fun s (k, v) -> s |> Map.add k v) hm

  let inline doRemove hm =
    removals |> Array.fold (fun s (k, _) -> s |> Map.remove k) hm

  let inline doLookup fa hm =
    fa |> Array.forall (fun (k, _) -> hm |> Map.containsKey k)
...
let testCases =
  if zoom_in then
    ...
    [| "Lookup", "Dictionary (Copy on Write)", TestDictionary.lookupInserted
       ...
       "Lookup",
       "Persistent Hash Map (F#)",
       TestPersistentHashMap.lookupInserted
       "Insert", "Persistent Hash Map (F#)", TestPersistentHashMap.insert
       "Remove", "Persistent Hash Map (F#)", TestPersistentHashMap.remove
