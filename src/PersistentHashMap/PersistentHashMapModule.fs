...
module PersistentHashMap =
  ...
  [<GeneralizableValue>]
  let empty<'K, 'V when 'K :> System.IEquatable<'K>> : PersistentHashMap<_, _> =
    upcast PersistentHashMap<'K, 'V>.Empty

  let inline isEmpty (m : PersistentHashMap<'K, 'V>) : bool = m.IsEmpty
